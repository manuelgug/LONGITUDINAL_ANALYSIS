...
merged_dfs <- merged_dfs[!merged_dfs$NIDA %in% samples_to_remove,]
length(unique(merged_dfs$NIDA))
...
#loci present in at least 95% of samples
threshold <- round(0.95 * length(unique(merged_dfs$NIDA))) 
...
merged_dfs_locifil_allefil <- merged_dfs_locifil[merged_dfs_locifil$norm.reads.locus >= 0.01,] #MAF
