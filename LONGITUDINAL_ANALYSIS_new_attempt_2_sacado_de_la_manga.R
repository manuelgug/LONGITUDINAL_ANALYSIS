...
# this version of the script inlcudes alfredo's suggestions:
# minimize the number of patients removed (excluded sample and patient filtering)
# use first and last visit: when possible to merge V1 and V5, do V1V5, else, keep whatever is available.
# use last visit
# compare just first and last visits
# try with 1 to n loci

#THIS APPROACH IS THE ONE AGRRED WITH ARNAU AND ARLINDO: uber conservative, err in the side of more R and less NI. 
# T1 USES LAX FILTER TO CAPTURE ALL POSSIBLE GENOMIC VARIATION
# T2 ONLY TAKES INTO ACCOUNT MAJOR ALLELE AS REAL, REMOEVES EVERYTHING ELSE. 
...
infection_analysis <- function(MAF = 0, min_reads = 100, number_of_loci = 1, get_outputs = F){ 
  ...
  merged_dfs_locifil_allefil <- merged_dfs %>%
    ...
    mutate(Visita = case_when(
      any(Visita == "V1") & any(Visita == "V5") ~ ifelse(Visita %in% c("V1", "V5"), "V1V5", Visita),
      TRUE ~ Visita
    )) %>%
    ungroup()
  ...
  valid_patients <- merged_dfs_locifil_allefil %>%
    ...
    pull(`Numero de estudo`) %>%
    unique()
  
  # Filter merged_dfs_modified to keep only valid Numero de estudo
  ...
  # Keep only the first and last unique Visita for each Numero de estudo
  merged_dfs_locifil_allefil <- merged_dfs_locifil_allefil %>%
    group_by(`Numero de estudo`) %>%
  ...
  filtered_data <- merged_dfs_locifil_allefil %>%
    filter(!Visita %in% c("V1", "V5", "V1V5")) %>%
  ...
  # threshold <- round(number_of_loci * length(unique(merged_dfs_locifil_allefil$NIDA))) 
  ...
  if (get_outputs == T){
    ...
    # free ram
    rm(csplot, stack_bar, number_of_loci)
    gc()
...
number_of_loci <- seq(1:100) #seq(1:length(unique(merged_dfs$locus)))
...
# Reverse the order of levels for number_of_loci
#infections_percentages$number_of_loci <- factor(infections_percentages$number_of_loci, levels = rev(levels(factor(infections_percentages$number_of_loci))))
infections_percentages <- infections_percentages[,-2]
